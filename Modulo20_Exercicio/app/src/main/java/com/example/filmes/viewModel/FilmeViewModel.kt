...
import com.example.filmes.helper.ImagemCarousel
...
class FilmeViewModel : ViewModel() {
    ...
    fun onFilmeSelected(position: Int) {
        ...
        filmesDetails?.let {
            viewModelScope.launch {
                ...
                val carouselImages =
                    it.id?.let { it1 -> ImagemCarousel.getCarouselImages(it1, filmesService)}
    ...
    private fun getFilmesData() {
        ...
        viewModelScope.launch {
            val response = filmesService.getLatesMovies(ApiCredentials.API_KEY)
